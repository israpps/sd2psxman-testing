...
McCommandHandler_t McCommandHandler = NULL;
...
int sd2psxman_sio2_send_port0(u8 in_size, u8 *in_buf)
{
    int rv;
...
int sd2psxman_sio2_send(int port, int slot, u8 in_size, u8 out_size, u8 *in_buf, u8 *out_buf)
{
    int rv;
    sio2_transfer_data_t sio2packet;
...
#define RETVAR pkt->ret
#define DEF_RET()
#define CREATE_RPC() if (!sd2psxman_CreateRPCService()) goto quit
#define STOP_RPC() sd2psxman_StopRPCService()
#else
#define CREATE_RPC()
#define STOP_RPC()
#define DEF_RET() int ret
#define RETVAR ret
#endif
...
int sd2psxman_ping(void *data)
{
    ...
    if (rdbuf[0x1] == SD2PSXMAN_REPLY_CONST) {
        RETVAR = rdbuf[0x3] << 16 | rdbuf[0x4] << 8 | rdbuf[0x5];
    } else {
...
int sd2psxman_send_raw_payload(void *data)
{
    ...
    return sd2psxman_sio2_send(pkt->port, pkt->slot, pkt->payload_size, pkt->payload_size, pkt->payload, pkt->payload);
}
